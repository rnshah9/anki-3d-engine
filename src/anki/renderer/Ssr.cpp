...
void Ssr::populateRenderGraph(RenderingContext& ctx)
{
	...
	rpass.newDependency({m_r->getDepthDownscale().getMinHiZRt(), TextureUsageBit::SAMPLED_COMPUTE, hizSubresource});
...
void Ssr::run(RenderPassWorkContext& rgraphCtx)
{
	...
	TextureSubresourceInfo hizSubresource;
	hizSubresource.m_mipmapCount = m_r->getDepthDownscale().getMipmapCount();
	rgraphCtx.bindTexture(0, 4, m_r->getDepthDownscale().getMinHiZRt(), hizSubresource);
